...
`com.apple.developer.dialing-app` **yönetilen bir Apple entitlement'ıdır**. Proje, onaylı Default Dialer derlemelerinde Ara düğmesine tek dokunuşla hücresel aramayı başlatmak için entitlement ve `DIALER_ENABLE_LIVE_COMMUNICATION_KIT` koşulu etkin olacak şekilde yapılandırılmıştır. Böylece iOS 26 ve sonrasında `tel:` onay çubuğu yerine LiveCommunicationKit kullanılır. Entitlement dosyasının projede bulunması Apple'ın bu yetkiyi hesabınıza verdiği anlamına gelmez; ücretsiz kişisel takım ya da sıradan provisioning profile ile imzalanamaz.
